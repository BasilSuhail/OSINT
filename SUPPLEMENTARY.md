...
Two real rows, straight out of `results/data/panel.csv`, same month, trimmed
...
NA       2022-03           0            —             —      -0.7301           0.4395
```

The first is Ukraine in the month of the invasion — two labels fired, 707
battle deaths, 4,406 political-violence deaths. The second is Namibia in the
same month, where nothing was labelled at all.

**The composite scores them 0.5543 and 0.4395.** An invasion is worth **0.11**
more than an ordinary month somewhere else.
...
Three things are visible in those two lines before any exam runs. `magnitude`
is **empty** where nothing was labelled, never zero. `signal_geo` is a
**z-score** — negative for below-normal — not a probability. And whatever the
composite is measuring, it is not the difference between those two months.
