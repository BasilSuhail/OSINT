...
/** Where anything docked to the bottom edge stops on a phone. The home
 *  indicator sits in that strip and is not a thing the page can draw over
 *  usefully — a card's last line under it is a line nobody reads, and a
 *  handle under it is one that swipes the app away instead of pressing. */
export const NARROW_COLUMN_BOTTOM = "calc(env(safe-area-inset-bottom) + 0.75rem)"
