...
import { clusterNews, newsSourceLabel, storyImpact, type Story } from "@/lib/newsClustering"
...
export function DashboardSection({ configured }: DashboardSectionProps) {
  ...
  /** Cross-outlet de-dup: collapse the per-article news rows into one Story per
   *  real-world event (title + shared-entity similarity). See lib/newsClustering. */
  const stories = useMemo<Story[]>(() => clusterNews(allNews), [allNews])
            ...
            <div className="flex flex-wrap items-center gap-1">
              {NEWS_FILTERS.map((f) => {
                ...
                const count =
                  f.key === "all"
                    ? stories.length
                    : stories.filter((s) => s.members.some(f.match)).length
                return (
                  <button
                    key={f.key}
                    type="button"
          ...
          {filteredStories.length === 0 ? (
            ...
            <ul className="grid max-h-[34rem] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2">
              {filteredStories.map((story) => {
                ...
                const clusterSize = story.outletCount
                const impact = storyImpact(story, Date.now())
                      ...
                      <div className="flex min-w-0 flex-1 flex-col">
                        ...
                        <div className="mt-auto flex flex-wrap items-center gap-x-2 gap-y-1 pt-2 font-mono text-[10px] uppercase tracking-wider text-neutral-500">
                          ...
                          <span
                            ...
                            title="impact = 0.35 outlet-pickup + 0.25 |sentiment| + 0.20 source + 0.20 recency"
                          >
                            impact {impact.toFixed(2)}
                          </span>
                          {clusterSize > 1 && (
                            <span
                              className="rounded border border-cyan-900/60 bg-cyan-950/30 px-1.5 py-0.5 text-cyan-300"
                              title={`Carried by ${clusterSize} outlets: ${story.outlets.join(", ")}`}
                            >
                              {clusterSize} sources · {story.outlets.slice(0, 2).join(" · ")}
