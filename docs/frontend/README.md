...
The production server mode configures a private Tailscale HTTPS origin during
`make serve-install`. Next handles `/api` on that same origin and forwards it
to the tailnet-bound backend. The equivalent ingress command is:

```bash
sudo tailscale serve --bg --yes --https=443 localhost:3000
```

Tailscale terminates TLS and proxies only to the loopback console; the console
then owns both page and API routing. Both pages can be installed independently
from the browser's Add to Home Screen or Install action.
