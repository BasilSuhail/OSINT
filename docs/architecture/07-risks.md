...
| **Pi 5 SD card wears out** | Low (over 10-week project window) | Moderate (boot lost; data safe on HDDs) | Boot from SSD if available; otherwise A2-rated SD; nightly `dd` of root partition to HDD |
...
| **Country panel selection biases evaluation results** | Medium | High (project credibility) | Country panel is fixed before any composite output is examined and committed to `config/country_panel.yaml`; LOOCV sensitivity test catches single-country dominance; panel stratified across geo / market / hazard event density |
...
| **Market data coverage gaps for emerging markets** | High | Moderate (P4 weak for EM countries) | Country panel stratified to include EM countries with adequate market-data coverage; document EM-coverage limitation explicitly in project Discussion |
...
| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| **Composite underperforms single-domain baselines** | Medium | Survivable (negative result is honest, defendable, and explicit in protocol) | Pre-registered protocol means this counts as a legitimate finding; project Discussion frames it constructively (what would multi-modal composites need to do differently). Per `05-originality.md` Claim 2 already commits to reporting this honestly. |
| **reader challenges JRC handbook fidelity** | Low | Moderate | Every JRC step is referenced in the Methods chapter; choices are justified by literature, not by what made the numbers look better |
...
10-week project window starting Week 2 of the term, hard deadline 28 August.

| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| **Pi setup / RAID tuning eats 2+ weeks** | Medium | High (everything else compresses) | Time-box hardware to one weekend; if blocked by Week 3, fall back to single 4TB drive + restic-only redundancy and revisit RAID post-project |
| **Layer 3 ambitions consume project-core time** | High (personal preference) | Catastrophic to assessment | **Hard rule: no Layer 3 worker is committed after end of Week 7.** Strict scope guard, documented here as the load-bearing rule of the project. Layer 3 PRs after Week 7 are politely closed without merge. |
| **Report scope creep — Tier 2 ends up in main body instead of appendix** | High | Moderate-to-High (word-budget burn) | Methods + Results chapters cover **Tier 1 only** (three composite domains + ground truth). Tier 2 (Layer 3) gets a single Discussion paragraph + appendix table. README documents this; draft review (Week 8) explicitly checks this. |
| **The reviewer unavailable for protocol lock-in meeting** | Low | High (evaluation cannot start) | Email The reviewer at end of Week 1 with the locked protocol draft and three meeting slots in Weeks 2-3; flag the dependency in the `methodology.md` open questions |
...
| **Review prep collides with writing tail** | High | High | First draft to The reviewer by Week 8 (mid-August), leaving Weeks 9-10 for review rehearsal and report polish |
...
| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| **ACLED ToS breach via inappropriate redistribution** | Low (with discipline) | High | ACLED data stored only in Parquet, never exposed on public dashboard endpoints, never committed to git; appendix cites ACLED per their citation policy |
| **GDELT redistribution interpreted as derivative work** | Low | Low | Only aggregate scores are shown publicly; raw GDELT rows never exposed; standard CC-BY attribution in project |
| **EM-DAT redistribution** | Low | Low | EM-DAT is freely available for academic use with citation; stored as Parquet, cited in project, not redistributed in raw form on the public dashboard |
...
| **Telegram OSINT scraping (Layer 3 idea) violates ToS** | High if attempted | Moderate | Telegram scraping is **not in scope** for the project; if added post-project, run with explicit consent / public channels only |
...
This file is not a prediction. It is a checklist for the regular project-management self-review (every two weeks, written into the review meeting notes). For each entry, the question is: is the mitigation still in place, and is the likelihood / impact still accurate? Update this file when answers change.

Two entries are **load-bearing for the assessment**:

1. **Layer 3 ambitions consume project-core time** — Week-7 hard-stop, documented here, no Layer 3 commits after that point.
2. **Report scope creep** — Methods + Results chapters cover Tier 1 only; Tier 2 lives in the appendix.
