"""Sensor cross-check worker body — verdicts for stories in the rolling window.

For every story still inside the clustering window, detect physical claims in
its member titles and check each claim against its sensor source (rules in
`app.corroboration.rules`). Called by the 30-minute beat task in `app.tasks`
and by `make sensor-checks`. Runs right after clustering beats so fresh
stories are checked while their sensor evidence (hazard retention ~2 days)
still exists.
"""
...
from app.db_models import EventRow, StoryMemberRow, StoryRow, StorySensorCheckRow
...
def _sensor_checks_inner(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    counters = {"stories": 0, "claims": 0, "confirmed": 0, "unconfirmed": 0, "kept_confirmed": 0}
