...
#: Rows per run. At the beat below this is ~5,760/day against a measured
#: 1,425 to 5,064 city-precision rows arriving per day, so the backlog drains
#: rather than grows, and a burst day is caught up on the next quiet one.
...
def enrich_titles(session: Session, *, limit: int = BATCH_LIMIT) -> dict[str, int]:
    ...
    fetchers on a small box, and twenty parallel requests to twenty news
    sites is a different kind of neighbour to be.
    """
    ids = pending_ids(session, limit=limit)
    if not ids:
        return {"considered": 0, "titled": 0, "failed": 0}
    ...
    with httpx.Client(
        ...
        for event in events:
            ...
            if result.title:
                ...
                failed += 1
    session.commit()
    return {"considered": len(events), "titled": titled, "failed": failed}
