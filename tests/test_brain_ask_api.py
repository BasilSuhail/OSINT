...
def test_ask_returns_sources_and_strips_bad_citations(monkeypatch):
    ...
    monkeypatch.setattr(
        api.qa,
        "build_qa_context",
        lambda session, question=None, history=None, exclude_story_ids=frozenset(), trace=None: {
            "stories": [
                {
                    "n": 1,
                    "story_id": 5,
...
def test_ask_threads_question_into_context_retrieval(monkeypatch):
    ...
    def _context(session, question=None, history=None, exclude_story_ids=frozenset(), trace=None):
...
def test_ask_regenerates_when_answer_echoes_history(monkeypatch):
    ...
    monkeypatch.setattr(
        api.qa,
        "build_qa_context",
        lambda session, **_kw: _ECHO_STORIES,
    )
    calls = iter(
        [
