...
class TestScheduled:
    ...
    def test_it_runs_on_the_analytics_queue(self):
        """It is an Ollama batch. On the default queue it would run alongside
        fetchers and contend with the brain for the model — the serialisation
        #388 exists to prevent."""
        from app.tasks import app

        route = app.conf.task_routes["app.tasks.grade_news_severity"]
