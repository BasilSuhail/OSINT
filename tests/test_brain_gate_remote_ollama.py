...
class TestWhereOllamaRuns:
    ...
    def test_anything_else_means_another_machine(self, url):
...
class TestTheContainerCase:
    def test_a_remote_model_is_not_blocked_by_local_memory(self, db_session: Session, monkeypatch):
    ...
    def test_the_reason_admits_the_floor_was_not_applied(self, db_session: Session, monkeypatch):
    ...
    def test_the_other_gates_still_apply_to_a_remote_model(self, db_session: Session, monkeypatch):
...
class TestAskTheBrain:
    def test_ask_was_refused_on_every_request_in_a_container(self, monkeypatch):
