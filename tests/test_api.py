...
def _positioned(source_event_id, lat, lon, *, category="hazard", occurred_at=None, severity=0.2):
    return EventRow(
        source="gdacs",
        ...
        lon=lon,
        keywords=[],
        payload={"title": "A hazard"},
    )


def test_event_grid_counts_by_cell(db_session):
    #: Two events in one 2-degree cell and one in another. A world view draws
    #: three markers here and needs three numbers, not three rows.
    client = _grid_rows(
        db_session,
        [
            _positioned("a", 10.1, 20.1),
            _positioned("b", 10.9, 20.9),
            _positioned("c", 40.5, 50.5),
        ],
    )
    body = client.get("/events/grid?cell_deg=2").json()
    cells = {(c["lat"], c["lon"]): c["count"] for c in body}
    assert cells == {(10.0, 20.0): 2, (40.0, 50.0): 1}


def test_event_grid_keys_cells_by_south_west_corner(db_session):
    #: Including south of the equator, where truncation toward zero would put a
    #: point in the cell above the one that contains it.
    client = _grid_rows(db_session, [_positioned("s", -3.5, -7.5)])
    cell = client.get("/events/grid?cell_deg=5").json()[0]
    assert (cell["lat"], cell["lon"]) == (-5.0, -10.0)
    assert cell["cell_deg"] == 5


def test_event_grid_separates_categories_and_reports_worst(db_session):
    client = _grid_rows(
        db_session,
        [
            _positioned("h", 1.0, 1.0, category="hazard", severity=0.9),
            _positioned("n", 1.5, 1.5, category="news", severity=0.1),
        ],
    )
    body = {c["category"]: c for c in client.get("/events/grid?cell_deg=2").json()}
    assert body["hazard"]["max_severity"] == pytest.approx(0.9)
    assert body["news"]["max_severity"] == pytest.approx(0.1)


def test_event_grid_honours_window_and_bbox(db_session):
    now = datetime.now(UTC)
    client = _grid_rows(
        db_session,
        [
            _positioned("in", 10.0, 20.0, occurred_at=now),
            _positioned("old", 10.0, 20.0, occurred_at=now - timedelta(days=30)),
            _positioned("far", -60.0, 150.0, occurred_at=now),
        ],
    )
    #: `+` means a space in a query string, so the offset goes as `Z`.
    since = (now - timedelta(days=3)).isoformat().replace("+00:00", "Z")
    body = client.get(
        f"/events/grid?since={since}&west=0&south=0&east=40&north=40&cell_deg=2"
    ).json()
    assert [(c["lat"], c["lon"], c["count"]) for c in body] == [(10.0, 20.0, 1)]


def test_event_grid_skips_rows_without_coordinates(db_session):
    ungeocoded = _positioned("nowhere", 0.0, 0.0)
    ungeocoded.lat = None
    ungeocoded.lon = None
    client = _grid_rows(db_session, [ungeocoded, _positioned("here", 5.0, 5.0)])
    body = client.get("/events/grid?cell_deg=2").json()
